...
def plot_2D_array(title, arr, xlabel="", xinterval=None, ylabel="", yinterval=None):
    return plot_2D_arrays(
        title, [arr], xlabel, xinterval, ylabel, yinterval, line_names=[]
    )
...
def benchmark(
    ...
    for i, timg in enumerate(trafoImages):
        ...
        d = features.FeatureMatcher.evaluateMatch(okps, tkps, matches, homographies[i])
        ds.append(d)

        isMatch, maxD = addROCData(okps, tkps, matches, homographies[i], matchThreshold)
