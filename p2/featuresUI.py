...
keypointClasses = [
    ("Harris", features.HarrisKeypointDetector),
    ("ORB", features.ORBKeypointDetector),
    ("Dummy", features.DummyKeypointDetector),
]
# Extra Credit not released for Spring 2022
...
matcherClasses = [
    ("Ratio Test", features.RatioFeatureMatcher),
...
class CustomJSONEncoder(json.JSONEncoder):
    ...
    def default(self, o):
        if (
            ...
            and hasattr(o, "size")
            and hasattr(o, "angle")
            and hasattr(o, "response")
            and hasattr(o, "octave")
            and hasattr(o, "class_id")
        ):
            return {
                "__type__": "cv2.KeyPoint",
                "point": o.pt,
                "size": o.size,
                "angle": o.angle,
                "response": o.response,
            ...
            hasattr(o, "distance")
            and hasattr(o, "trainIdx")
            and hasattr(o, "queryIdx")
            and hasattr(o, "imgIdx")
        ):
            return {
                "__type__": "cv2.DMatch",
                "distance": o.distance,
...
class ImageWidget(tk.Canvas):
    ...
    def fitImageToCanvas(self, cvImage):
        ...
        dest = cv2.resize(cvImage, (width, height), interpolation=cv2.INTER_LANCZOS4)
        return dest
...
class BaseFrame(tk.Frame):
    ...
    def __init__(self, parent, root):
        ...
        self.keypointOptions = tk.OptionMenu(
            *((self, self.keypointTypeVar) + tuple(x[0] for x in keypointClasses))
        )
        ...
        self.matcherOptions = tk.OptionMenu(
            *((self, self.matcherTypeVar) + tuple(x[0] for x in matcherClasses))
        )
    ...
    def getSelectedDetector(self):
        index = [x[0] for x in keypointClasses].index(self.keypointTypeVar.get())
        keypointClass = keypointClasses[index][1]
        return keypointClass()

    def getSelectedDescriptor(self):
        index = [x[0] for x in descriptorClasses].index(self.descriptorTypeVar.get())
...
class KeypointDetectionFrame(BaseFrame):
    def __init__(self, parent, root):
        ...
        self.loadImageButton = tk.Button(
            self, text="Load Image", command=self.loadImage, width=BUTTON_WIDTH
        )
        self.loadImageButton.grid(row=0, column=0, sticky=tk.W + tk.E)

        self.clearKeypointsButton = tk.Button(
            self, text="Clear Keypoints", command=self.reloadImage, width=BUTTON_WIDTH
        )
        self.clearKeypointsButton.grid(row=0, column=1, sticky=tk.W + tk.E)

        self.screenshotButton = tk.Button(
            self, text="Screenshot", command=self.screenshot, width=BUTTON_WIDTH
        )
        self.screenshotButton.grid(row=0, column=4, sticky=tk.W + tk.E)

        self.computeButton = tk.Button(
            self, text="Compute", command=self.computeKeypoints, width=BUTTON_WIDTH
        )
        ...
        self.thresholdSlider.grid(row=1, column=4, columnspan=2, sticky=tk.W + tk.E)
    ...
    def drawKeypoints(self, val=None):
        ...
        if self.image is not None and self.keypoints is not None:
            ...
            self.setStatus("Found " + str(len(kps)) + " keypoints")
    ...
    def screenshot(self):
        if self.image is not None:
            filename = tkFileDialog.asksaveasfilename(
                parent=self.root, filetypes=supportedFiletypes, defaultextension=".png"
            )
            if filename:
...
class FeatureMatchingFrame(BaseFrame):
    def __init__(self, parent, root):
        ...
        self.loadLeftImageButton = tk.Button(
            self, text="Load Left Image", command=self.loadLeftImage, width=BUTTON_WIDTH
        )
        self.loadLeftImageButton.grid(row=0, column=0, sticky=tk.W + tk.E)

        self.loadRightImageButton = tk.Button(
            self,
            text="Load Right Image",
            command=self.loadRightImage,
            width=BUTTON_WIDTH,
        )
        self.loadRightImageButton.grid(row=0, column=1, sticky=tk.W + tk.E)

        self.clearMatchingsButton = tk.Button(
            self,
            text="Clear Matchings",
            command=self.clearMatchings,
            width=BUTTON_WIDTH,
        )
        ...
        self.computeMatchesButton = tk.Button(
            self, text="Compute", command=self.computeMatchesClick, width=BUTTON_WIDTH
        )
        ...
        self.thresholdSlider.grid(row=2, column=1, columnspan=2, sticky=tk.W + tk.E)
        self.thresholdSlider.bind("<ButtonRelease-1>", self.thresholdChanged)

        tk.Label(self, text="Percent Matches:").grid(row=2, column=3, sticky=tk.W)

        self.percentMatches = tk.Scale(
            self,
            from_=0,
            to=100,
            resolution=0.1,
        ...
        self.percentMatches.grid(row=2, column=4, columnspan=2, sticky=tk.W + tk.E)
    ...
    def updateMatchCount(self, percent):
        if self.matches is not None:
            ...
            if len(matches) == 0:
                ...
                matchImage = self.drawMatches(
                    self.image[0],
                    self.thresholdedKeypoints[0],
                    self.image[1],
                    self.thresholdedKeypoints[1],
                    matches,
                )
    ...
    def thresholdAndMatch(self):
        ...
        for i in range(2):
            ...
            self.descriptors[i] = descriptor.describeFeatures(
                self.image[i], self.thresholdedKeypoints[i]
            )
...
class BenchmarkFrame(BaseFrame):
    def __init__(self, parent, root):
        ...
        self.thresholdSlider.grid(row=0, column=1, columnspan=2, sticky=tk.W + tk.E)
        ...
        self.runBenchmarkButton = tk.Button(
            self,
            text="Run benchmark",
            command=self.runBenchmarkClick,
            width=BUTTON_WIDTH,
        )
