...
class ConvolutionTestCase(unittest.TestCase):
    ...
    def testValues(self):
        ...
        output = student.convolve(self.random_img_3d, self.random_filt)
        output_gt = convolve(
            self.random_img_3d, self.random_filt[:, :, None], mode="constant"
        )
        self.assertTrue(np.allclose(output, output_gt), "Incorrect values")
...
class GradientTestCase(unittest.TestCase):
    ...
    def testValues(self):
        ...
        test_mag = np.array([0.46888004, 0.22153629, 0.14217979, 0.0, 0.0605548])
        test_ori = np.array([0.78539816, 3.14159265, -2.35619449, 0.0, 0.78539816])
        self.assertTrue(
            np.allclose(dmag, test_mag, rtol=1e-5), "Gradient magnitude does not match"
        )
        self.assertTrue(
            np.allclose(dori, test_ori, rtol=1e-5),
...
class Check_distance_from_lineTestCase(unittest.TestCase):
    ...
    def testValues(self):
        ...
        self.assertTrue(
            np.all(out[:3] == True) and np.all(out[3:] == False),
            "Check distance from line incorrect.",
        )
...
class Draw_linesTestCase(unittest.TestCase):
    def setUp(self):
        self.img = student.imread("test.png")
        self.lines = [
            [-1.57079633, 274.89321881],
            [-1.02101761, -11.10678119],
            [3.14159265, 68.89321881],
        ]
    ...
    def testValues(self):
        ...
        self.assertTrue(
            (275, 100) in red_idx and (275, 300) in red_idx, "Missing red line."
        )
        self.assertTrue(
            (51, 104) in red_idx and (156, 276) in red_idx, "Missing red line."
        )
        self.assertTrue(
            (100, 69) in red_idx and (300, 69) in red_idx, "Missing red line."
        )
