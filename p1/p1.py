...
def convolve(img, filt):
    ...
    padded = np.pad(img, [((filt_h-1)//2, (filt_h-1)//2), ((filt_w-1)//2, (filt_w-1)//2), (0, 0)], "constant")
    filtered = np.zero((height, width, channels))
...
def gaussian_filter(k, sigma):
    ...
    for x in range(-center, center + 1):
        for y in range(-center, center + 1):
            gauss_filter[x + center,  y + center] = np.exp(-(x**2 + y**2)/(2*sigma**2))
